...
// JavaScript Document
function myCheckLeavingPage(txt, base) {
    var yes = confirm(txt);
    if (yes) {
        return true;
    } else {
...
function call_ajax(id, value) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                if (value == 5) {
                    document.getElementById(eID).setAttribute("class", "red");
                } else {
                    document.getElementById(eID).setAttribute("class", "black");
                }

            }
        }
    }
    xmlhttp.open("GET", "ajax.php?status=" + value + "&comp_id=" + id, true);
    try{
        //Set XHR header
...
function call_ajax_person(id, value) {
    var xmlhttp;
    var eID = "row_" + id;
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
        xmlhttp = new XMLHttpRequest();
    ...
    xmlhttp.open("GET", "ajax.php?status=" + value + "&user_id=" + id, true);
    try{
        //Set XHR header
        xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    }catch(e){
...
//Waits a split secont to prevent too many requests going through at once
var timer;

queueRequest = function(makeRequest) {
...
function call_ajax_sample_size(get,id,idd){
	
    if (!window.jQuery) {
        var jq = document.createElement('script'); jq.type = 'text/javascript';
        // Path to jquery.js file, eg. Google hosted version
        jq.src = '/includes/js/jquery-1.9.1.js';
        document.getElementsByTagName('head')[0].appendChild(jq);
	}

    // Make it slightly easier to manage
    
    var loadingGif = '<img height="12px" src="/includes/images/small-ajax2.gif" / >';
    
    document.getElementById(idd).innerHTML = loadingGif;
    document.getElementById(id).innerHTML = loadingGif;

    queueRequest(function(){
        
        return $.get("bmcriteria_ajax.php?"+get,{},function(samples){
	    	var arr = samples.split("#");
	    	document.getElementById(idd).innerHTML = arr[0];
	    	document.getElementById(id).innerHTML = arr[1];
        });
	
    });
}

function call_ajax_add_countries(get, id) {
    var xmlhttp;
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
        xmlhttp = new XMLHttpRequest();
    }
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            var samples = xmlhttp.responseText;

            //alert(samples);

            var arr = samples.split("#");
            document.getElementById(id).innerHTML = arr[0];
            document.getElementById('hiddenRegion').value = arr[1];
            document.getElementById('hiddenCountry').value = arr[2];
            getSampleSize();
            //document.forms[0].selected_country_regions.focus();
        }
    }

    xmlhttp.open("POST", "addremovecountries_ajax.php?" + get, true);
    try{
        //Set XHR header
        xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    }catch(e){
...
/*
 function call_ajax_probe_sic(code,lang,id){
 alert(code+" : "+lang+" : "+id);
 var xmlhttp;
 if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
 xmlhttp=new XMLHttpRequest();
 ...
 xmlhttp.open("POST","sic_codes_probe.php?code="+code+"&lang="+lang,true);
 xmlhttp.send(null);
 }
 */
function call_ajax_probe_sic(code, lang, id) {
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            //alert(xmlhttp.responseText);
            document.getElementById('probe_sic_select_' + (id + 1)).innerHTML = xmlhttp.responseText;
        }
    }
    if (id < 4) {
        for (i = 1; i < 4; i++) {
            document.getElementById('probe_sic_select_' + i).name = 'probe_sic_select_' + i;
        }
    }
    for (i = (id + 1); i < 4; i++) {
        document.getElementById('probe_sic_select_' + i).innerHTML = '';
    }
    xmlhttp.open("POST", "sic_codes_probe.php?code=" + code + "&lang=" + lang + "&id=" + id, true);
    try{
        //Set XHR header
        xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    }catch(e){
...
function call_ajax_probe_sic_select(code, lang, id, select) {
    //alert(code+" : "+lang+" : "+id);
    var xmlhttp;
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
        xmlhttp = new XMLHttpRequest();
    ...
    xmlhttp.open("POST", "sic_codes_probe.php?code=" + code + "&lang=" + lang + "&id=" + id + "&select=" + select, true);
    try{
        //Set XHR header
        xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    }catch(e){
...
function call_ajax_sic_code(get, id) {
    var xmlhttp;
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
        xmlhttp = new XMLHttpRequest();
    }
    ...
    xmlhttp.open("POST", "sic_codes_ajax.php?" + get, true);
    try{
        //Set XHR header
        xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
    }catch(e){
...
function get_benchmark(){
	...
	xmlhttp.onreadystatechange=function(){
	  	if (xmlhttp.readyState==4 && xmlhttp.status==200){
	  		...
	  		window.location = "/benchmark/finance.php" //---------------------ADD back in
	  		//document.getElementById(id).innerHTML = xmlhttp.responseText;
		}
	}
	xmlhttp.open("POST","benchmark_api.php?doBM=1",true);
        try{
            //Set XHR header
            xmlhttp.setRequestHeader("X-Requested-With", "XMLHttpRequest");
        }catch(e){
...
function call_ajax_sic_code_option(_value, id) {
...
function phil() {
    for (inst in tinyMCE.editors) {
        if (tinyMCE.editors[inst].getContent)
            {
                tinyMCE.editors[inst].setContent("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam vitae nunc molestie, fringilla metus nec, fringilla risus. Sed mi ipsum, consectetur eget risus in, imperdiet pretium nibh. Nam eget fringilla justo. Nulla facilisi. Morbi et nisi sem. Aenean vel vulputate lorem, quis faucibus sapien. Proin ultricies viverra tincidunt. Proin non ultrices est, id luctus eros. Nam nec scelerisque mauris. Integer vel felis ac elit aliquet pretium ut id dolor. Sed consectetur volutpat purus, id ultrices sem tincidunt a. Integer vitae placerat tortor. Nullam eu sagittis lorem. Proin sit amet magna placerat, dictum risus tempus, auctor sapien. Aliquam posuere nunc a bibendum bibendum. In nec nisl eget erat adipiscing tristique. Phasellus consequat malesuada eros at lacinia.");
                tinyMCE.editors[inst].save();
            }
...
function philfinance()
{
    $('input[type=text]').each(function(){$(this).val(Math.floor(Math.random() * 10000));
